    ...
    public function updatedLiquidationId($value)
    {
        if ($value) {
            ...
            if ($liquidation) {
                ...
                $this->amountReturned = number_format($this->amountToReturn, 2);
                dd($this->amountReturned);
            }
        }
    }
...
<div>
    <div class="flex justify-between">
        <x-ts-breadcrumbs separator="icon:chevron-right" :items="[
            ...
            ['label' => 'View cash return - Event liquidation', 'icon' => 'eye'],
        ]" class="mb-3" />
    </div>
        ...
        <x-ts-card>
            ...
            <x-slot:footer>
                <div class="flex justify-end gap-2">
                    ...
                    @if($status === 'DRAFT') 
                        <x-ts-button icon="pencil-square"  :href="route('cash-return.event-crs.edit', $cashReturnId)">Edit</x-ts-button>
                    @else 
                        <x-ts-button icon="pencil-square" outline disabled>Edit</x-ts-button>
