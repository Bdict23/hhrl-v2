...
new class extends Component
{
    ...
    public function fetchData()
    {
        ...
        //check if the pcv status is closed, if closed redirect back to summary page with error message
        if ($pettyCashVoucherData->status == 'CLOSED') {
    ...
    public function update(CashReturnService $service)
    {
        try {
            ...
            // 4. Call the Service
            $crs = $service->update($data);

            // 5. Success Feedback
            $this->toast()->success('Success', "Cash Return {$crs->reference} updated successfully!")->send();
            $this->reset();
...
<div>
    <div class="flex justify-between">
        <x-ts-breadcrumbs separator="icon:chevron-right" :items="[
                              ...
                              ['label' => 'PCV Cash return edit', 'icon' => 'pencil-square'],
                  ]"  class="mb-3"/>
    </div>
    ...
    <div class="grid gap-4">
        ...
        <x-ts-card>
            ...
            <div class="grid grid-cols-2 w-full p-3 gap-3">
                ...
                <div>
                    <x-ts-currency label="Return Amount" wire:model="amountReturned" mutate symbol :disabled="$source == 'REVOLVING'"/>
                    <x-ts-textarea label="Notes" resize maxlength="250" count placeholder="Add note here..." wire:model="notes"/>
                    <div class="grid grid-cols-3 gap-2 mt-3">
                        <x-ts-stats number="{{$returnAmount}}" title="FOR RETURN" >
                            <x-slot:icon>
    ...
    <x-ts-modal wire="errorModal" persistent size="md" center>
        <div class="p-5 items-center text-center">
            <x-ts-icon name="x-circle" class="w-16 h-16 text-rose-500 mx-auto mb-4" />
            <h2 class="text-3xl font-semibold">Modification failed</h2>
            <p class="mt-2">Associated PCV is already closed</p>
            <div class="mt-10 w-full">
                <x-ts-button md :href="route('cash-return.summary-tab')" icon="chevron-left">Back</x-ts-button>
            </div>
        </div>
    </x-ts-modal>
    <x-ts-back-to-top />
