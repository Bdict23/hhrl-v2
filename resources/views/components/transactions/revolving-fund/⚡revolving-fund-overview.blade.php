...
new class extends Component
{
    ...
    public $revolvingFundId, $balance = 0, $reference, $ceilingAmount, $maxEpenditurePercentage, $replenishedAmount, $expensedAmount = 0, $expenditurePercentage;
    ...
    public function mount()
    {
        ...
        $this->expensedAmount = RevolvingFundService::expensedAmount(Auth::user()->branch_id);
    ...
    public function replenish(RevolvingFundService $revolvingFundService)
    {
        ...
        try{
            ...
            $this->toast()->success('Success', "Revolving Fund Batch {$po->reference} genearated successfully!, with replenished amount of ₱{$po->replenished_amount}")->send();
            $this->reset();
            $this->mount();
        }catch(Exception $e){
             \Log::error("PCV Creation Failed: " . $e->getMessage());
            $this->toast()->error('Error', 'Something went wrong while generate new batch: ' . $e->getMessage())->send();
        }
    ...
    public function with(): array
    {
        ...
        return [
            ...
            'batchHistoryHeader' => [
                ...
                ['index' => 'replenished_amount', 'label' => 'replenished amount' , 'sortable' => false],
                ['index' => 'starting_balance', 'label' => 'starting balance' , 'sortable' => false],
                ['index' => 'ending_balance', 'label' => 'Ending balance',  'sortable' => false],
                ['index' => 'opened_at', 'label' => 'Opened date'],
                ['index' => 'closed_at', 'label' => 'closed date'],
            ],


            'rows' => RevolvingFundDetail::query()
                ->where('revolving_fund_id', $this->revolvingFundId)
                ->where('status', 'FINAL')
   ...
   <x-ts-tab selected="Active">
     <x-ts-tab.items tab="Active">
            ...
            <x-ts-card class=" p-6">
                <div class="flex justify-between items-start">
                    ...
                    <div>
                        ...
                        <x-ts-button icon="arrow-path" wire:click="$toggle('replenishModal')" loading>Replenish & Rollover</x-ts-button>
                </div>
            ...
            <x-ts-modal id="modal-ids"  wire="replenishModal" z-index="z-10" center>
                <x-ts-card>
                    <x-ts-select.styled
                                :request="route('api.get.active.acknowledgement-for-revolving-fund', ['branch_id' => auth()->user()->branch_id ])"
                                select="label:reference|value:id|description:additional_details"
                                label="Acknowledgement"
                                wire:model.live='acknowlegementId'
                                :placeholders="[
                                'default' => 'Select',
                                'empty'   => 'No acknowledgement found',
                                ]" required
                                id="acknowledgementSelect"
                                />
     ...
     <x-ts-tab.items tab="Batch History">
            <x-ts-table :headers="$batchHistoryHeader" :rows="$batchHistoryRows" striped :sort  expandable>
                ...
                @php
                    $insertRow = [];
                    foreach ($row->revolvingFundDetail as $index => $value) {
                        $insertRow[] = [
