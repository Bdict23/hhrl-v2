...
new class extends Component
{
    ...
    public function with(): array
    {
        return [
           ...
           'withdrawalHeader' => [
                ...
                ['index' => 'receiving_status', 'label' => 'status'],
                ['index' => 'created_at', 'label' => 'Date' ],
                ['index' => 'total_received_amount', 'label' => 'received amount' ],
    ...
    public function reviewedAction(): void
    {
        ...
        $this->dialog()
        ->question('Approve liquidation?', 'Proceeding will complete the liquidation.')
        ->confirm(
            'Confirm',
            'approval', //pass a functio to call
            )
        ->cancel('Cancel')
        ->send();
    }
    ...
    public function approval(EventLiquidationService $service)
    {
        try {
            ...
            $this->reset();
            $this->dialog()
            ->success('Success', "event liquidation {$po->reference} approved successfully!")
            ->flash() 
            ->send();
            return redirect()->route('event-liquidation.validation-summary');
            
        } catch (\Exception $e) {
            // Log the error if needed
            \Log::error("Liquidation approval Failed: " . $e->getMessage());
            $this->toast()->error('Error', 'Something went wrong while approving: ' . $e->getMessage())->send();
        }
    }
...
<div>
    <div class="flex justify-between">
        <x-ts-breadcrumbs separator="icon:chevron-right" :items="[
                              ...
                              ['label' => 'View Event Liquidation - Approval', 'icon' => 'eye'],
                  ]"  class="mb-3"/>
        ...
        <x-ts-card>
            ...
            <x-slot:footer>
                <div class="flex justify-start gap-2">
                     ...
                     <x-ts-button  icon="check" outline wire:click="reviewedAction">APPROVE AND VALIDATED</x-ts-button>
