...
<div>
    <div class="lg:flex lg:justify-between grid mb-4">
        <x-ts-breadcrumbs separator="icon:chevron-right" :items="[
            ...
            ['label' => 'Event Booking - Edit', 'icon' => 'list-bullet'],
            ]"/>
                                ...
                                <div class="grid grid-cols-2 gap-6">
                                    ...
                                    <div class="grid gap-2 grid-cols-2">
                                       ...
                                       <div class="col-span-2 h-fit flex justify-end gap-2">
                                            <x-ts-dropdown>
                                                <x-slot:action>
                                                    <x-ts-button x-on:click="show = !show" md icon="chevron-down" position="right" loading>UPDATE
                                                        AS</x-ts-button>
                                                </x-slot:action>
                                                <x-ts-dropdown.items outline icon="archive-box-arrow-down" text="DRAFT"
                                                    wire:click="updateEventAsDraftAction" />
                                                <x-ts-dropdown.items icon="clipboard-document-check" text="FINAL" separator
                                                    wire:click="updateEventAsFinalAction" />
                                            </x-ts-dropdown>
                                       </div>
                                    </div>
                                </div>
                                @error('selectedVenueRows.*')
                                    <x-ts-alert title="Error on Venue Selection" text="{{ $message }}" color="red" light bordered="left" rounded="xl"/>
                                @enderror
                                @error('selectedServiceRows.*')
                                    <x-ts-alert title="Error on Service Selection" text="{{ $message }}" color="red" light bordered="left" rounded="xl"/>
                                @enderror
                                @error('selectedFoodRows.*')
                                    <x-ts-alert title="Error on Food Selection" text="{{ $message }}" color="red" light bordered="left" rounded="xl"/>
                                @enderror
                            </x-ts-card>
                       </div>
                    </x-ts-step.items>
