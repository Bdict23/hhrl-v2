...
new class extends Component
{
    ...
    public function markAsRevise(): void
    {
        ...
        ->confirm(
            'Confirm',
            'approveAction', //pass a functio to call
            )
        ->cancel('Cancel')
        ->send();
    }
    public function markAsApproved(): void
    {
        ...
        // 2. show confirmation dialog
         $this->dialog()
        ->question('Approve Withdrawal?', 'Are you sure to approve this withdrawal ?')
        ->confirm(
            'Confirm',
            'approveAction', //pass a function to call
            )
        ->cancel('Cancel')
        ->send();
    }
    public function approveAction(WithdrawalService $service)
    {
        try {
            ...
            $this->toast()->error('Error', 'Something went wrong while approving the withdrawal: ' . $e->getMessage())->send();
        }
    }
                ...
                <div class="grid gap-2 p-3">
                    ...
                    <x-ts-stats number="{{ number_format($grand_total, 2) }}" title="Total Cost">
                            ...
                            <x-slot:right>
                                    ...
                                    <x-ts-button icon="check" flat class="underline" wire:click="markAsApproved">Approve</x-ts-button>
                            </x-slot:right>
                    </x-ts-stats>
                </div>
                <div class="grid gap-2 p-3">
