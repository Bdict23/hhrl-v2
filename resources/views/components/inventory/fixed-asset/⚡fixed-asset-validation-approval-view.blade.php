...
new class extends Component
{
   ...
   public function mount($id)
    ...
    public function approveAction(){
        $this->dialog()
        ->question('Update Status', 'Mark this batch as approved ?')
        ->confirm(
            'Yes! Confirm',
            'applyApprove',
            )
    ...
    public function applyApprove(FixedAssetService $service){
        try {
            $service->batchApproved($this->batchId);
            $this->toast()->success('Success', 'Asset Batch has been marked as approved successfully!')->send();
            //refresh data to reflect changes
            $this->loadData();
        } catch (\Exception $e) {
           $this->toast()->error('Error', 'Something went wrong while saving: ' . $e->getMessage())->send();
...
<div>
   <div class="flex justify-between">
        <x-ts-breadcrumbs separator="icon:chevron-right" :items="[
                              ...
                              ['label' => 'Fixed Asset Approval', 'icon' => 'pencil-square'],
                  ]"  class="mb-3"/>
    </div>
        ...
        <x-ts-card>
            <div class="grid grid-cols-3 w-full">
                <div class="grid gap-3 p-2">
                    <x-ts-input label="Purchase Order Reference" :value="$header->purchaseOrder->requisition_number" readonly/>
            ...
            <div class="grid grid-cols-2">
                <div class="grid gap-2 p-3">
                    ...
                    <x-ts-stats :number="$grand_total" title="Total Cost" animated>
                                ...
                                <x-slot:right>
                                    <x-ts-button icon="check" flat class="underline" wire:click="approveAction" :disabled="$step != 3">Approved</x-ts-button>
                                    <x-ts-button icon="arrow-path-rounded-square" wire:click="reviseAction" flat class="underline" :disabled="$step != 3">Revise</x-ts-button>
                                 </x-slot:right>
